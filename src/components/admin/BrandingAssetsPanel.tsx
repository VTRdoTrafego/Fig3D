  ...
  return (
    <Card className="space-y-3 rounded-2xl border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] p-3">
      ...
      <div className="flex h-24 items-center justify-center overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(24,25,29,0.86),rgba(14,15,19,0.92))]">
        {previewUrl ? (
          <img src={previewUrl} alt={label} className="h-full w-full object-contain" loading="lazy" decoding="async" />
        ) : (
  ...
  const saveLogoAsset = async (field: LogoField, file: File) => {
    ...
    try {
      ...
      if (field === 'primaryLogoUrl') {
        patch.headerLogoUrl = config.headerLogoUrl || dataUrl
        patch.splashLogoUrl = config.splashLogoUrl || dataUrl
        ...
        <div className="mt-4 space-y-5">
          <section className="space-y-3">
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              ...
              <Input
                ...
                onChange={(event) => patchTextField('shortDescription', event.target.value)}
                placeholder="Descricao curta"
              />
              <Input
                className="sm:col-span-2"
                value={config.marketingDescription}
                onChange={(event) => patchTextField('marketingDescription', event.target.value)}
                placeholder="Descricao comercial para landing"
              />
              ...
              <label className="flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                <input
                  ...
                  onChange={(event) => patchToggle('published', event.target.checked)}
                />
          ...
          <section className="space-y-3">
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              <UploadTile
                ...
                hint="Usada como base em areas configuradas do app."
                previewUrl={config.primaryLogoUrl}
                onUpload={(file) => saveLogoAsset('primaryLogoUrl', file)}
                onRemove={() => removeLogoAsset('primaryLogoUrl')}
                disabled={savingField === 'primaryLogoUrl'}
              />
              <UploadTile
                label="Favicon"
                hint="Pode ser automatico a partir da logo principal."
                previewUrl={config.faviconUrl}
                onUpload={(file) => saveLogoAsset('faviconUrl', file)}
                onRemove={() => removeLogoAsset('faviconUrl')}
                disabled={savingField === 'faviconUrl'}
              />
              <UploadTile
                label="Logo de abertura / splash"
                hint="Exibida no destaque inicial da landing."
                previewUrl={config.splashLogoUrl}
                onUpload={(file) => saveLogoAsset('splashLogoUrl', file)}
                onRemove={() => removeLogoAsset('splashLogoUrl')}
                disabled={savingField === 'splashLogoUrl'}
              />
              <UploadTile
                label="Logo de header/navbar"
                hint="Exibida no topo e navegacao."
                previewUrl={config.headerLogoUrl}
                onUpload={(file) => saveLogoAsset('headerLogoUrl', file)}
                onRemove={() => removeLogoAsset('headerLogoUrl')}
                disabled={savingField === 'headerLogoUrl'}
              />
            </div>
            <div className="grid gap-2 sm:grid-cols-2">
              <label className="flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={config.useLogoAsFavicon}
                  onChange={(event) => patchToggle('useLogoAsFavicon', event.target.checked)}
                />
                Usar logo principal como favicon
              </label>
              <label className="flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={config.useLogoInSplash}
                  onChange={(event) => patchToggle('useLogoInSplash', event.target.checked)}
                />
                Usar logo na abertura
              </label>
              <label className="flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={config.useLogoInHeader}
                  onChange={(event) => patchToggle('useLogoInHeader', event.target.checked)}
                />
                Usar logo no header/navbar
              </label>
              <label className="flex items-center gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                <input
                  type="checkbox"
                  checked={config.useLogoInPublicPages}
                  onChange={(event) => patchToggle('useLogoInPublicPages', event.target.checked)}
                />
                Usar logo nas paginas publicas
              </label>
            </div>
          </section>
          ...
          <section className="space-y-3">
            <h3 className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--text-secondary)]">
              Modelos nos 4 circulos da landing
            </h3>
            <p className="text-xs text-[var(--text-muted)]">
              Um arquivo por circulo (ex.: cada formato/estilo do app). Sem upload, o circulo usa o GIF de{' '}
              <span className="text-[var(--text-secondary)]">Logo de abertura / splash</span>.
            </p>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <UploadTile
                label="Circulo 1"
                hint="Primeiro tipo demonstrado na landing."
                previewUrl={config.demoModel1Url}
                onUpload={(file) => saveDemoModelAsset('demoModel1Url', file)}
                onRemove={() => setConfigPatch({ demoModel1Url: null })}
                disabled={savingField === 'demoModel1Url'}
              />
              <UploadTile
                label="Circulo 2"
                hint="Segundo tipo demonstrado na landing."
                previewUrl={config.demoModel2Url}
                onUpload={(file) => saveDemoModelAsset('demoModel2Url', file)}
                onRemove={() => setConfigPatch({ demoModel2Url: null })}
                disabled={savingField === 'demoModel2Url'}
              />
              <UploadTile
                label="Circulo 3"
                hint="Terceiro tipo demonstrado na landing."
                previewUrl={config.demoModel3Url}
                onUpload={(file) => saveDemoModelAsset('demoModel3Url', file)}
                onRemove={() => setConfigPatch({ demoModel3Url: null })}
                disabled={savingField === 'demoModel3Url'}
              />
              <UploadTile
                label="Circulo 4"
                hint="Quarto tipo demonstrado na landing."
                previewUrl={config.demoModel4Url}
                onUpload={(file) => saveDemoModelAsset('demoModel4Url', file)}
                onRemove={() => setConfigPatch({ demoModel4Url: null })}
                disabled={savingField === 'demoModel4Url'}
              />
            </div>
          </section>
              ...
              {[
                ...
                <Card key={preview.id} className="rounded-2xl border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] p-3">
                  ...
                  <div className="mt-2 flex h-20 items-center justify-center overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[radial-gradient(120px_90px_at_50%_18%,rgba(109,75,255,0.18),rgba(11,12,16,0.86))]">
                    <img src={preview.url} alt={preview.label} className="h-full w-full object-contain" loading="lazy" decoding="async" />
                  </div>
                </Card>
              ))}
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="hidden"
                  onChange={async (event) => {
                    await addDemoAssets(event.target.files)
                    event.currentTarget.value = ''
                  }}
                />
                <Button size="sm" variant="secondary" disabled={savingField === 'marketingDemoAssets'} onClick={() => demoInputRef.current?.click()}>
                  <ImagePlus size={14} />
                  Enviar demonstracoes
                </Button>
              </div>
            </div>
            ...
            {featuredAsset ? (
              <Card className="rounded-2xl border-[rgba(245,196,0,0.3)] bg-[linear-gradient(180deg,rgba(245,196,0,0.08),rgba(255,255,255,0.03))] p-3">
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-[120px_1fr]">
                  <div className="h-[88px] overflow-hidden rounded-xl border border-[var(--border-soft)] bg-[rgba(0,0,0,0.22)]">
                    <img src={featuredAsset.url} alt={featuredAsset.name} className="h-full w-full object-cover" loading="lazy" decoding="async" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-semibold text-[var(--text-primary)]">{featuredAsset.name}</p>
                    <p className="text-xs text-[var(--text-secondary)]">
                      Usado automaticamente no bloco de demonstracao da landing para reforco de conversao.
                    </p>
              ...
              <Card className="rounded-2xl border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] p-3">
                <p className="text-xs text-[var(--text-muted)]">Nenhum asset enviado. Use o botao de upload para adicionar demonstracoes.</p>
              </Card>
            )}
            ...
            <div className="grid gap-2">
              {config.marketingDemoAssets.map((asset, index) => (
                <Card key={asset.id} className="rounded-2xl border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="h-14 w-14 overflow-hidden rounded-lg border border-[var(--border-soft)] bg-[rgba(0,0,0,0.25)]">
                      <img src={asset.url} alt={asset.name} className="h-full w-full object-cover" loading="lazy" decoding="async" />
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1">
                      <Button size="sm" variant="ghost" onClick={() => moveAsset(index, index - 1)} disabled={index === 0}>
                        <MoveUp size={13} />
                      </Button>
                      <Button
                        size="sm"
                        ...
                        disabled={index === config.marketingDemoAssets.length - 1}
                      >
                        <MoveDown size={13} />
                      </Button>
                      <Button
                        size="sm"
                        variant={asset.featured ? 'premium' : 'secondary'}
                        onClick={() => setFeaturedAsset(asset.id)}
                      >
                        <Star size={13} />
                        {asset.featured ? 'Destaque' : 'Destacar'}
                      </Button>
                      <label
                        className={cn(
                          'inline-flex items-center gap-1 rounded-xl border px-2 py-1 text-[11px]',
                          asset.published
                            ? 'border-[rgba(34,197,94,0.45)] bg-[rgba(34,197,94,0.14)] text-emerald-200'
                            : 'border-[var(--border-soft)] bg-[rgba(255,255,255,0.04)] text-[var(--text-secondary)]',
                        )}
                      >
                        <input
                          type="checkbox"
                          checked={asset.published}
                          onChange={(event) => toggleAssetPublished(asset.id, event.target.checked)}
                        />
                        Publicar
                      </label>
                      <Button size="sm" variant="ghost" onClick={() => removeDemoAsset(asset.id)}>
                        <Trash2 size={13} />
                      </Button>
                    </div>
                  </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[var(--border-soft)] bg-[rgba(255,255,255,0.03)] p-3">
            <p className="text-xs text-[var(--text-secondary)]">
              Ultima atualizacao: {new Date(config.updatedAt).toLocaleString('pt-BR')}
            </p>
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  resetConfig()
                  toast.success('Branding restaurado para o padrao.')
                }}
              >
                <RefreshCcw size={13} />
                Resetar branding
